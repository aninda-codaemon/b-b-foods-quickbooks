...
/**
 * Build a request to import purchase orders already in QuickBooks into our database
 */
...
{	
	...
	if ($Doc = $Parser->parse($errnum, $errmsg))
	{
		...
		foreach ($List->children() as $PurchaseOrder)
		{
			...
			mysqli_query($dblink, "
				DELETE FROM qb_recent_purchaseorder_purchaseorderline WHERE PurchaseOrder_TxnID = '" . mysqli_real_escape_string($dblink, $arr['TxnID']) . "'
			...
			foreach ($PurchaseOrder->children() as $Child)
			{
				if ($Child->name() == 'PurchaseOrderLineRet')
				{
					...
					// Store the po line items in MySQL
					mysqli_query($dblink, "
						REPLACE INTO
						qb_recent_purchaseorder_purchaseorderline
						(
