...
if(mysqli_num_rows($query) > 0) {
    ...
    while ($row = mysqli_fetch_array($query)) {
        mysqli_query($dblink, "
                INSERT INTO
                qb_error_log
                (`ListID`, `Module`, `Msg`) VALUES ('".$row['ListID']."', 'datatransfercustomer', 'cron test')");

        //echo "ListID : ".$row['ListID']." <br><br>";
        
        $sql_row_maintbl = "SELECT * FROM `qb_customer` where `ListID` = '".$row['ListID']."'";	
        $query_row_maintbl = mysqli_query($dblink, $sql_row_maintbl);

        $sql_row_tmptbl = "SELECT * FROM `qb_example_customer` where `ListID` = '".$row['ListID']."'";	
        ...
        if(mysqli_num_rows($query_row_maintbl) > 0){
            ...
            if(!empty($diff)){
                ...
                //echo '<pre>';
                //print_r($diff);
                //echo "update into qb_customer : iteration - $iteration <br><br>";
                foreach($diff as $key=>$val)
                {
                    $val =  mysqli_real_escape_string($dblink, $val);
                    mysqli_query($dblink, "
                    UPDATE qb_customer SET ". $key . " = '". $val ."' WHERE `ListID` = '".$result_row_tmptbl['ListID']."'");
                }
            }else{
                //echo "equal : iteration - $iteration <br><br>" ;
            }
        }else{
            //echo "insert into qb_customer : iteration - $iteration <br><br>";
